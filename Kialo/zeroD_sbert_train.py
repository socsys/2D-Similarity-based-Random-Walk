...
from zeroD_SoftmaxLossBert import *
...
for e in range(3):
    ...
    dense_model = Dense.Dense(in_features=1*word_embedding_model.get_word_embedding_dimension(), out_features=2)
    ...
    for i in range(len(trainset)):
        texts=[trainset.iloc[i]['sent' + str(1)]+' '+trainset.iloc[i]['sent' + str(2)]] # appending first at the end u
    ...
    for i in range(len(testset)):
        texts=[testset.iloc[i]['sent' + str(1)]+' '+testset.iloc[i]['sent' + str(2)]] # appending first at the end u
