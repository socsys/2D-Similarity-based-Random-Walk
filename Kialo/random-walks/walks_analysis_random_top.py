...
def top_L_relevant_utterances(sentences, data, node_id, L):
    ...
    print("size of all nodes",len(candidates))
...
for i in range(1, walk_length + 1):
    ...
    cols.append(f'sent{i}')      # Sentence
cols.append('label')
pd.DataFrame(train_samples2, columns=cols).to_csv('Kialo/Classification/random-walks/random_GK_train_random_walk.csv', index=False)
pd.DataFrame(dev_samples2, columns=cols).to_csv('Kialo/Classification/random-walks/random_GK_test_random_walk.csv', index=False)
...
for i in range(1, walk_length + 1):
    ...
    cols.append(f'sent{i}')      # Sentence
cols.append('label')
pd.DataFrame(train_samples3, columns=cols).to_csv('Kialo/Classification/random-walks/top_GK_train_random_walk.csv', index=False)
pd.DataFrame(dev_samples3, columns=cols).to_csv('Kialo/Classification/random-walks/top_GK_test_random_walk.csv', index=False)
