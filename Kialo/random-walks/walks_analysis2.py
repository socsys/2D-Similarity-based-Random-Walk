...
def plot_comparison_bar():
    ...
    with professional error bars (mean ± std)."""
    ...
    bars_2d = ax.bar(
        ...
        yerr=std_scores_2d,
        error_kw=dict(
            ecolor='#2F2F2F',   # neutral, Nature-style
            elinewidth=2,
            capsize=6,
            capthick=2
        )
    ...
    # Save
    plt.savefig('Kialo/Classification/random-walks/Kialo_scores_bar_plot.png', dpi=800)
    plt.savefig('Kialo/Classification/random-walks/Kialo_scores_bar_plot.pdf', format='pdf', dpi=1200)
...
def plot_average_values_categorical(dic, name):
    """
    Plot a 2D bar chart of average values for categorical labels
    with professional error bars (mean ± std).
    """
    ...
    for label, values in dic.items():
        if len(values) >= population:
            categories.append(label_map[label])
            averages.append(np.mean(values))
            stds.append(np.std(values))

    # Fixed colors (Nature-style, consistent)
    category_colors = {
        '1D': '#55A868',
        '2D': '#C44E52'
    }
    ...
    bar_colors = [category_colors.get(cat, '#BBBBBB') for cat in categories]
    ...
    # Save
    plt.savefig(f'Kialo/Classification/random-walks/{name}_average_values2.png', dpi=800)
    plt.savefig(f'Kialo/Classification/random-walks/{name}_average_values2.pdf', format='pdf', dpi=1200)
