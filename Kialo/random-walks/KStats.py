...
def format_k(n):
    if n >= 1_000_000:
        return f"{n/1_000_000:.1f}M"  # one decimal
    elif n >= 1_000:
...
# Paths
# ==============================
KIALO_PATH = 'Kialo/Classification/random-walks/serializedGraphs/'
OUTPUT_DIR = 'Kialo/Classification/compare2/random-walks/stats'
...
# ==============================
def compute_graph_stats_kialo(data):
    """
    Compute structural statistics for a Kialo discourse graph.
    Uses data.node and data.edge attributes.
    Only counts nodes that actually branch for breadth metrics.
    """
    G = nx.DiGraph()
    ...
    # Breadth: only consider nodes with out-degree > 0
...
files = os.listdir(KIALO_PATH)
print(f"Processing {len(files)} Kialo graphs...")
...
for indx, file in enumerate(files):
    ...
    with open(os.path.join(KIALO_PATH, file), "rb") as f:
        data = pkl.load(f)

    # Compute and store per-graph statistics
    stats = compute_graph_stats_kialo(data)
    stats["graph_id"] = file
    ...
    for node_id in data.node.keys():
        ...
        print("processing",indx)
        print('length of samples', len(dataset_samples))
...
df.to_csv(os.path.join(OUTPUT_DIR, "Kialo_graph_statistics_raw.csv"), index=False)
...
summary.to_csv(os.path.join(OUTPUT_DIR, "Kialo_graph_statistics_summary.csv"), index=False)
...
axes[0, 0].yaxis.set_major_formatter(plt.FuncFormatter(lambda x, _: format_k(int(x))))
...
axes[0, 1].yaxis.set_major_formatter(plt.FuncFormatter(lambda x, _: format_k(int(x))))
...
axes[1, 0].yaxis.set_major_formatter(plt.FuncFormatter(lambda x, _: format_k(int(x))))
...
axes[1, 1].yaxis.set_major_formatter(plt.FuncFormatter(lambda x, _: format_k(int(x))))
...
plt.tight_layout()
plt.savefig(os.path.join(OUTPUT_DIR, 'Kialo_depth_branching_histograms.png'), dpi=300)
plt.savefig(os.path.join(OUTPUT_DIR, 'Kialo_depth_branching_histograms.pdf'), dpi=1000, format="pdf",bbox_inches='tight')
...
ttest_file = "Kialo/Classification/compare2/random-walks/stats/paired_ttest_results.txt"
