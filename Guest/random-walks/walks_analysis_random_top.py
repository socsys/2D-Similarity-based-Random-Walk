...
def random_L_utterances(sentences, data, node_id, L):
    """
    Baseline (i): Random L utterances from the graph.
...
def visualize_graph_walk(tree_data, chosen_node_ids, extended_edges, start_node_id, label):
    ...
    folder_path = 'Guest/random-walks/' + str(label)
...
def plot_combined_cdf():
    ...
    cdf_file_path = 'Guest/random-walks/combined_cdf.png'
...
def plot_combined_pdf():
    ...
    pdf_file_path = 'Guest/random-walks/combined_pdf.png'
...
def plot_histogram(dic,name):
    ...
    pdf_file_path = 'Guest/random-walks/'+name+'_pdf.png'
    plt.savefig(pdf_file_path)
    plt.show()
...
def plot_pdf(dic,name):
    ...
    pdf_file_path = 'Guest/random-walks/' + name + '_pdf.pdf'
...
def plot_pdf_all(all, name, pop=1000):
    ...
    pdf_file_path = 'Guest/random-walks/' + name + '_all_pdf.png'
    plt.savefig(pdf_file_path, dpi=800)

    pdf_file_path = 'Guest/random-walks/' + name + '_all_pdf.pdf'
    plt.savefig(pdf_file_path,format='pdf', dpi=1200)
...
def plot_cdf(dic,name):
    ...
    cdf_file_path = 'Guest/random-walks/'+name+'_cdf.png'
    plt.savefig(cdf_file_path,dpi=800)

    cdf_file_path = 'Guest/random-walks/'+name+'_cdf.pdf'
...
def plot_interpolated_cdf(dic, name, num_interp_points=500):
    ...
    cdf_file_path = f'Guest/random-walks/{name}_interpolated_cdf.png'
    plt.savefig(cdf_file_path, dpi=800)

    cdf_file_path = f'Guest/random-walks/{name}_interpolated_cdf.pdf'
    plt.savefig(cdf_file_path, format='pdf',dpi=1200)
...
def plot_comparison_bar():
    ...
    bar_file_path = f'Guest/random-walks/scores_bar_plot.png'
    plt.savefig(bar_file_path, dpi=800)

    bar_file_path = f'Guest/random-walks/scores_bar_plot.pdf'
...
def plot_average_values_categorical(dic, name):
    ...
    avg_file_path_png = f'Guest/random-walks/{name}_average_values.png'
    plt.savefig(avg_file_path_png, dpi=800)

    avg_file_path_pdf = f'Guest/random-walks/{name}_average_values.pdf'
...
def plot_whisker_all(data, name):
    ...
    plt.tight_layout()
    plt.savefig(f'Guest/random-walks/{name}_boxplot.png', dpi=800)
    plt.savefig(f'Guest/random-walks/{name}_boxplot.pdf', format='pdf', dpi=1200)
...
def plot_two_whiskers(data1, name1, data2, name2):
    ...
    bp = plt.boxplot(data,
                     ...
                     medianprops=dict(color=median_color1, linewidth=2, linestyle='--', zorder=4),
    ...
    # Save and display
    plt.tight_layout()
    plt.savefig(f'Guest/random-walks/2D_vs_1D_boxplot_g.png', dpi=800)
    plt.savefig(f'Guest/random-walks/2D_vs_1D_boxplot_g.pdf', format='pdf', dpi=1200)
    plt.show(block=False)
    plt.pause(1)
    plt.close()
...
for indx, file in enumerate(files):
    if indx >= 0:
        ...
        for node_id in data['node'].keys():
            ...
            #random
            sentences2= ['']*walk_length*3
            sentences2, label2,chosen_node_ids2, extended_edges2,score2=random_L_utterances(sentences2, data, node_id, walk_length)

            #top similar
            sentences3= ['']*walk_length*3
            sentences3, label3,chosen_node_ids3, extended_edges3,score3=top_L_relevant_utterances(sentences3, data, node_id, walk_length)
            ...
            if label != -1 and label!='' and not sentences[5]=='' and not sentences1[5]=='':
                ...
                sentences2.append(label2)
                sentences3.append(label3)

                dataset_samples.append(sentences)
                dataset_samples1.append(sentences1)
                dataset_samples2.append(sentences2)
                dataset_samples3.append(sentences3)
...
pd.DataFrame(train_samples, columns=cols).to_csv('Guest/random-walks/GG_train_random_walk.csv', index=False)
pd.DataFrame(dev_samples, columns=cols).to_csv('Guest/random-walks/GG_test_random_walk.csv', index=False)
...
for i in range(1, walk_length + 1):
    ...
    cols.append(f'sent{i}')      # Sentence
cols.append('label')
# pd.DataFrame(train_samples1, columns=cols).to_csv('Guest/random-walks/one_GG_train_simil_random_walk.csv', index=False)
# pd.DataFrame(dev_samples1, columns=cols).to_csv('Guest/random-walks/one_GG_test_simil_random_walk.csv', index=False)
pd.DataFrame(train_samples1, columns=cols).to_csv('Guest/random-walks/one_GG_train_random_walk.csv', index=False)
pd.DataFrame(dev_samples1, columns=cols).to_csv('Guest/random-walks/one_GG_test_random_walk.csv', index=False)
...
print('#samples Random:', len(dataset_samples2))
random.seed(12345)
random.shuffle(dataset_samples2)

train_samples2 = dataset_samples2[ : math.ceil(0.8*len(dataset_samples2))]
dev_samples2 = dataset_samples2[math.ceil(0.8*len(dataset_samples2)) : ]
cols = []
for i in range(1, walk_length + 1):
    cols.append(f'parent_id{i}')  # Parent ID for each sentence
    cols.append(f'id{i}')        # ID for each sentence
    cols.append(f'sent{i}')      # Sentence
cols.append('label')
pd.DataFrame(train_samples2, columns=cols).to_csv('Guest/random-walks/random_GG_train_random_walk.csv', index=False)
pd.DataFrame(dev_samples2, columns=cols).to_csv('Guest/random-walks/random_GG_test_random_walk.csv', index=False)


print('#train samples Random:', len(train_samples2))
print('#test samples Random:', len(dev_samples2))



print('#samples Top L:', len(dataset_samples3))
random.seed(12345)
random.shuffle(dataset_samples3)

train_samples3 = dataset_samples3[ : math.ceil(0.8*len(dataset_samples3))]
dev_samples3 = dataset_samples3[math.ceil(0.8*len(dataset_samples3)) : ]
cols = []
for i in range(1, walk_length + 1):
    cols.append(f'parent_id{i}')  # Parent ID for each sentence
    cols.append(f'id{i}')        # ID for each sentence
    cols.append(f'sent{i}')      # Sentence
cols.append('label')
pd.DataFrame(train_samples3, columns=cols).to_csv('Guest/random-walks/top_GG_train_random_walk.csv', index=False)
pd.DataFrame(dev_samples3, columns=cols).to_csv('Guest/random-walks/top_GG_test_random_walk.csv', index=False)


print('#train samples Top L:', len(train_samples3))
print('#test samples Top L:', len(dev_samples3))