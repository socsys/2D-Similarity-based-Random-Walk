...
for e in range(3):
    ...
    trainset = pd.read_csv('G_train_random_walk.csv')
    trainset = trainset.fillna('')

    for i in range(len(trainset)):
        context=""
        for j in range(2, 11):
            context+= trainset.iloc[i]['sent' + str(j)]
            if j<10:
                context+=' [SEP] '
        texts=[context, trainset.iloc[i]['sent' + str(1)] ] # appending first at the end u
    ...
    testset = pd.read_csv('G_test_random_walk.csv')
    testset = testset.fillna('')

    for i in range(len(testset)):
        context=""
        for j in range(2, 11):
            context+= testset.iloc[i]['sent' + str(j)]
            if j<10:
                context+=' [SEP] '
        texts=[context, testset.iloc[i]['sent' + str(1)] ] # appending first at the end u
