...
df_train = pd.read_csv('two_G_train_random_walk.csv').fillna('')
df_test  = pd.read_csv('two_G_test_random_walk.csv').fillna('')
...
for e in range(num_folds):
    ...
    for i in range(len(train_df)):
        context=""
        for j in range(2, 11):
            context+= train_df.iloc[i]['sent' + str(j)]
            if j<10:
                context+=' [SEP] '
        texts=[context, train_df.iloc[i]['sent' + str(1)] ] # appending first at the end u
    ...
    for i in range(len(test_df)):
        context=""
        for j in range(2, 11):
            context+= test_df.iloc[i]['sent' + str(j)]
            if j<10:
                context+=' [SEP] '
        texts=[context, test_df.iloc[i]['sent' + str(1)] ] # appending first at the end u
    ...
    model_save_path = 'output/sbert_hate_attention_' + model_name.replace("/", "-")
    ...
    dense_model = Dense.Dense(in_features=3*word_embedding_model.get_word_embedding_dimension(), out_features=2)
