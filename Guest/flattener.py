...
for idx, row in df.iterrows():
    ...
    for parent, node, comment in triples:
        ...
        if parent:
            ...
            G.add_node(node)  # maybe orphan root
    ...
    output_rows.append([first_comment]+ linearized_comments + [label])


with open("LG_test_random_walk.csv", "w", newline='', encoding='utf-8') as f:
    writer = csv.writer(f)
    writer.writerow(['Utterance']+ ['Context_'+str(i) for i in range(1,11)]+ ["Label"])
